...
export function Notebook() {
  const { clues, collectedRunes, hasUnread, markAsRead } = useGame();
  const [isOpen, setIsOpen] = useState(false);

  // 如果没有任何线索和碎片，且未打开过，可以考虑隐藏，但为了引导玩家，我们常驻显示
  if (clues.length === 0 && collectedRunes.length === 0 && !isOpen) {
    return null;
  }
        ...
        {collectedRunes.length > 0 && (
          <div className="mb-6 animate-in fade-in slide-in-from-left-4">
            <div className="flex items-center gap-2 mb-2 border-b border-[#8b7355]/30 pb-1">
              ...
              <h3 className="font-bold text-[#5c4033] text-sm">化煞符文碎片 ({collectedRunes.length}/7)</h3>
            </div>
            <div className="flex gap-2 flex-wrap">
              {[1, 2, 3, 4, 5, 6, 7].map(num => {
                const runeId = `RUNE_0${num}` as any;
                const hasGot = collectedRunes.includes(runeId);
                return (
                  <div
                    key={num}
                    className={`w-8 h-8 flex items-center justify-center border ${hasGot ? 'border-amber-600 bg-amber-100 text-amber-800 font-bold shadow-inner' : 'border-dashed border-[#8b7355]/30 text-transparent'}`}
                  >
                    {hasGot ? '符' : '?'}
                  </div>
                );
